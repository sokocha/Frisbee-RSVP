...
/**
 * Calculate access period that closes 24 hours before game time
 * and opens right after the close time (so it reopens immediately after closing)
 *
 * Example: Game on Sunday at 17:00
 * - Close: Saturday at 17:00 (24h before)
 * - Open: Saturday at 17:01 (right after close, so list is open immediately)
 */
function calculateAccessPeriod(gameDay, gameStartHour, gameStartMinute, timezone) {
  // Calculate 24 hours before game = same time, previous day
  let closeDay = gameDay - 1;
  if (closeDay < 0) closeDay = 6; // Wrap to Saturday if game is Sunday

  const closeHour = gameStartHour;
 ...
 */
export async function createOrganization({ slug, name, sport, location, timezone, ownerId, maxParticipants, gameSchedule, organizerEmail }) {
  // Validate slug
  ...
  const organization = {
    ...
    visibility: 'private', // private | public - private by default
    createdAt: new Date().toISOString(),
  };
  ...
  // Calculate smart access period: closes 24h before game, opens right after
  ...
  const accessPeriod = calculateAccessPeriod(gameDay, gameStartHour, gameStartMinute, timezone);
...
export async function updateOrganization(id, updates) {
  ...
  const allowedFields = ['slug', 'name', 'sport', 'location', 'timezone', 'status', 'displayOrder', 'visibility'];
