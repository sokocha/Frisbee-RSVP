...
/**
 * Calculate access period based on schedule settings
 *
 * Supports both:
 * - Presets: 'always-open', '6-hours', '12-hours', '24-hours' (relative to game time)
 * - Custom: Explicit rsvpOpenDay/Hour/Minute and rsvpCloseDay/Hour/Minute
 *
 * When using presets:
 * - Close time = game start time minus hours from preset
 * - Open time = 1 minute after game ends (for next week signups)
 */
function calculateAccessPeriod(schedule, timezone) {
  const {
    gameDay,
    gameStartHour,
    gameStartMinute,
    ...
    gameEndMinute,
    rsvpWindowPreset,
    // Custom timing (optional - overrides preset)
    rsvpOpenDay,
    rsvpOpenHour,
    rsvpOpenMinute,
    rsvpCloseDay,
  ...
  // Check if we have explicit custom timing
  const hasCustomTiming = rsvpOpenDay !== undefined && rsvpCloseDay !== undefined;

  if (hasCustomTiming) {
    // Use explicit timing provided by user
    return {
      enabled: true,
      startDay: rsvpOpenDay,
      startHour: rsvpOpenHour ?? 0,
      startMinute: rsvpOpenMinute ?? 0,
      endDay: rsvpCloseDay,
      endHour: rsvpCloseHour ?? 0,
      endMinute: rsvpCloseMinute ?? 0,
      timezone: timezone || 'Africa/Lagos',
    };
  }
  ...
  // Open time is 1 minute after the game ends
  // This lets people sign up for next week right after this week's game
  let openMinute = gameEndMinute + 1;
  let openHour = gameEndHour;
  let openDay = gameDay;
...
export async function createOrganization({ slug, name, sport, location, streetAddress, timezone, ownerId, maxParticipants, gameSchedule, organizerEmail, rsvpWindowPreset }) {
  ...
  // Calculate access period based on RSVP window preset or custom timing
  ...
  const gameEndHour = gameSchedule?.endHour ?? 19;
  const gameEndMinute = gameSchedule?.endMinute ?? 0;
  const accessPeriod = calculateAccessPeriod({
    ...
    gameEndMinute,
    rsvpWindowPreset,
    // Pass through custom timing if provided
    rsvpOpenDay: gameSchedule?.rsvpOpenDay,
    rsvpOpenHour: gameSchedule?.rsvpOpenHour,
    rsvpOpenMinute: gameSchedule?.rsvpOpenMinute,
    rsvpCloseDay: gameSchedule?.rsvpCloseDay,
    rsvpCloseHour: gameSchedule?.rsvpCloseHour,
    rsvpCloseMinute: gameSchedule?.rsvpCloseMinute,
  }, timezone);
  ...
  const settings = {
    ...
    gameInfo: {
      enabled: true,
      recurrence: gameSchedule?.recurrence || 'weekly',
      monthlyOccurrence: gameSchedule?.monthlyOccurrence || null,
      gameDay: gameDay,
