 ...
 *
 * Opens 1 minute after the game ends (so people can sign up for next week right away)
 * Example: Tuesday 2-4pm game with 12-hour preset
 *   - Opens: Tuesday 4:01pm (right after game ends)
 *   - Closes: Tuesday 2:00am (12 hours before next game)
 *   - This gives people almost a full week to sign up
 */
function calculateAccessPeriod(gameDay, gameStartHour, gameStartMinute, gameEndHour, gameEndMinute, timezone, rsvpWindowPreset = '6-hours') {
...
export async function createOrganization({ slug, name, sport, location, streetAddress, timezone, ownerId, maxParticipants, gameSchedule, organizerEmail, rsvpWindowPreset }) {
  ...
  const accessPeriod = calculateAccessPeriod(gameDay, gameStartHour, gameStartMinute, gameEndHour, gameEndMinute, timezone, rsvpWindowPreset);
