...
export function isFormOpenWeekly(settings) {
  ...
  // When open, compute the close time (dropout deadline)
  let closeTime = null;
  if (isOpen) {
    let daysUntilClose = endDay - currentDay;
    if (daysUntilClose < 0) daysUntilClose += 7;
    if (daysUntilClose === 0 && (currentHour > endHour || (currentHour === endHour && currentMinute >= endMinute))) {
      daysUntilClose += 7;
    }
